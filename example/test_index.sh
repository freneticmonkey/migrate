...
echo ">>>>>>> Empty Metadata <<<<<<<"
docker exec -t -i example_management_db_1 mysql -ptest -e 'select * from metadata;' management

echo ">>>>>>> Diffing YAML Schema - Should Generate Create Tables <<<<<<<"
./migrate diff

echo ">>>>>>> Migrating <<<<<<<"
./migrate --verbose sandbox --migrate
