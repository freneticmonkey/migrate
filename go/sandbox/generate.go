...
func Generate(conf config.Config, tableName string) (err error) {
	...
	templateFile := util.WorkingSubDir(conf.Options.Template.File)

	// Generation path
	genPath := util.WorkingSubDir(conf.Options.Template.Path)
	...
	for _, tbl := range yaml.Schema {
		...
		tblFilename := fmt.Sprintf("%s.%s", strings.ToLower(tbl.Name), conf.Options.Template.Ext)
